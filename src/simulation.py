...
class Simulation:
    def __init__(self, **kwargs):
        if not("trace" in kwargs or "det" in kwargs):
            ...
            self.det = kwargs["det"]
            self.delta, self.alpha, self.beta, self.gamma = self.generate_integer_matrix()
        ...
        self.d = 1
        self.thresholds = np.array([self.u_thold, self.v_thold, self.ut_thold, self.vt_thold, self.d])
        

        self.S0 = np.array([1, 1, 3, 3])

        self.guesses = [[self.alpha0], [self.beta0], [self.delta0], [self.gamma0]]
        self.parms = np.array([self.alpha, self.beta, self.delta, self.gamma, self.alpha0, self.beta0, self.delta0, self.gamma0], dtype=np.float64)
        ...
        self.time_tracker = [self.last_updates, self.time_between, self.tfe]
    ...
    def update_parms(self, update_idx, S, pos_err):
        ...
        if abs(u)/abs(x) <= u_thold and abs(v)/abs(y) <= v_thold:
            ...
            for i in update_idx:
                parm_idx = i + 4
                self.guesses[i].append(parm_idx)
    ...
    def run(self):
        ...
        create_static_figure(sol, guesses, self.true_vals, derivs, self.t, self.updates_to_plot)
