...
def create_static_figure(
    sol, guesses, true_vals, derivs, t, updates_to_plot, display_on=True
):
    """
    ...
    if trace <= 0 and determinant > 0:  # Plot absolute errors for stable cases
        ...
        ax[1, 1].plot(
            t, relut, "k", label=r"$\frac{|\dot{\tilde{x}} - \dot{x}|}{|\dot{x}|}$"
        )
        ax[1, 1].plot(
            t, relvt, "C0", label=r"$\frac{|\dot{\tilde{y}} - \dot{y}|}{|\dot{y}|}$"
        )
        ax[1, 1].set_yscale("log")
    ...
    if display_on:
        ...
        anim_limits = [xmin, xmax, ymin, ymax]
        return anim_limits


def create_system_animation(sol, guesses, true_vals, derivs, t):
    """
    ...
    anim_limits = create_static_figure(
        sol, guesses, true_vals, derivs, t, display_on=False
    )
    xmin, xmax, ymin, ymax = anim_limits
    plt.xlim(xmin, xmax)
    plt.ylim(ymin, ymax)
